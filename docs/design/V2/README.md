...
| Collapsible nested sections ("Tone Curve", "Color Grading", "Optics") | `egui::CollapsingHeader` |
...
- Left panel: 216px fixed width, `#171717` bg, 1px `#262626` right border.
  ...
  - FOLDERS section: flat filesystem tree, e.g. "▸ 100MSDCF 3333" (count right-aligned, monospace, `#666`).
  - COLLECTIONS section (with a "+" add button): **nested tree** — collection *sets* (e.g. "Trips", "Client Work") shown with a ▾/▸ chevron and indented leaf collections beneath (e.g. "Japan 2025", "Iceland — Highlands", "Sent 2026"); flat collections (e.g. "Portfolio") sit at the top level with a small dot icon instead of a chevron. Counts right-aligned.
...
**Metadata Filters popup** (anchored under the "Metadata" button, right-aligned to viewport): `#1d1d1d` bg, 1px `#353535` border, 300px wide. Header "METADATA FILTERS" + "Reset" link. Combo rows: Camera, Lens, Rating (active filters get accent-tinted border/text). "FILE TYPE" segmented chips: RAW/JPEG/HEIC/TIFF. "EXPOSURE RANGE": ISO/Aperture/Focal sliders. Footer: "Apply Filters" (accent-filled) / "Close" buttons.
...
- Top filmstrip: 96px tall horizontal scroll strip of 120px-wide thumbnails, `#171717` bg. Selected thumbnail gets a 2px accent outline.
- Center canvas: fills remaining width, `#0d0d0d` bg, image centered/contained (max 94% of available space).
  ...
  - **Floating EXIF chip** (bottom-left over canvas): stacked monospace lines — focal length, aperture, shutter, ISO, zoom%.
  - **Info toggle button** (bottom-left over canvas, stacked ABOVE the EXIF chip so they never overlap — positioned ~132px from the bottom vs. the chip's 14px): "ℹ Info" pill button, highlights when the Info panel (see below) is open.
  - **Crop mode overlay** (only when Crop tool active): rule-of-thirds grid lines + 8 corner/edge handle squares drawn directly on the canvas.
...
- **Right panel** (300px, `#1a1a1a` bg, 1px `#262626` left border): header always shows camera name + edit status ("No edits"/"Saved") + a "Working space" combo (e.g. "Rec2020"). Below that, content swaps entirely based on the active tool:
  ...
  **Tool: Adjust** (default) and **Tool: Mask** *share the exact same "options library"* — same 3 tabs, same nested collapsible sections, same slider set — the only differences are (a) Mask's panel is prefixed with a mask-management header block, and (b) Mask's slider values start at 0 (untouched) since they're scoped per-mask rather than global.
  ...
  - **Effects tab**: a "SHARPENING" section label, then Amount/Radius/Detail sliders; a divider; a "NOISE REDUCTION" section label (with a small "AI" chip hinting at a future AI-denoise option) containing Luminance, Detail, Color, and **Color Detail** (chroma noise reduction) sliders — then a collapsible **"Optics"** section containing lens name + "Choose lens…" button, Distortion (with an enable checkbox + "Needs a matched lens" note when no profile matched), and Vignette (manual). Footer: Reset all.

  **Tool: Crop** — tabs disappear entirely, replaced by a dedicated panel: "CROP & TRANSFORM" label, Angle slider, Aspect combo ("Original") + a wrapping row of compact direct-format chips (Original/1:1/4:3/3:2/16:9/5:4/Custom — selected chip gets accent tint), "Reset crop" button, then a "GEOMETRY" section: Keystone V/H sliders, "Auto Perspective" / "Guided Upright" buttons, Reset/Reset all footer.
...
- **Tool switching** (Adjust/Crop/Heal/Mask floating cluster): click to switch; changes the entire right-panel content and adds mode-specific canvas overlays (crop grid, mask tint). Only one tool active at a time.
- **Collapsible sections** ("Tone Curve", "Color Grading", "Optics" inside each tab): click the row to toggle open/closed; chevron flips ▸ → ▾. Only relevant when their parent tab is active.
...
- No drag-and-drop, animation, or transition effects were designed — everything is instant state changes, consistent with egui's immediate-mode redraw model.
...
- Per-tab collapsible-open booleans (curve/grade/optics) — tracked **separately** for the Adjust tool vs. the Mask tool, since a mask's disclosure state shouldn't affect the global Adjust panel's
