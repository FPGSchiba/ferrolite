...
// looks up the radial gain in the baked LUT, and multiplies rgb by
// `mix(1.0, gain, vig_amount)`. At `vig_amount == 0` this is the identity
// (rgb unchanged), which keeps existing goldens byte-identical.
//
...
// the Rust `VignetteUniform { vig_amount: f32, pad: [f32; 3] }`. A `vec3<f32>`
// member would force 16-byte alignment and bloat the struct to 32 bytes.
struct V {
    vig_amount: f32,
    pad0: f32,
    pad1: f32,
